...
public class ACSManage {
    ...
    public static void acsCfg(int lUserID) {
        ...
        if (!b_GetAcsCfg) {
            log.debug("获取门禁主机参数，错误码为{}", AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            log.debug("获取门禁主机参数成功");
            struAcsCfg.read();
            log.debug("1.是否显示抓拍图片：{}", struAcsCfg.byShowCapPic);  //是否显示抓拍图片， 0-不显示，1-显示
            log.debug("2.是否显示卡号：{}", struAcsCfg.byShowCardNo);   //是否显示卡号，0-不显示，1-显示
            log.debug("3.是否开启语音提示：{}", struAcsCfg.byVoicePrompt);  //是否启用语音提示，0-不启用，1-启用
            log.debug("4.联动抓图是否上传：{}", struAcsCfg.byUploadCapPic); //联动抓拍是否上传图片，0-不上传，1-上传
        }
        ...
        if (!b_SetAcsCfg) {
            log.debug("设置门禁主机参数，错误码为：{}", AcsMain.hCNetSDK.NET_DVR_GetLastError());
        } else {
            log.debug("设置门禁主机参数成功！！！");
        }
    }
    ...
    public static void getAcsStatus(int lUserID) {
        ...
        if (!b_GetAcsStatus) {
            log.debug("获取门禁主机工作状态，错误码为：{}", AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            log.debug("获取门禁主机工作状态成功！！！");
            netDvrAcsWorkStatusV50.read();
            log.debug("1.门锁状态（或者梯控的继电器开合状态）：{}", netDvrAcsWorkStatusV50.byDoorLockStatus[0]); //门锁状态：0- 正常关，1- 正常开，2- 短路报警，3- 断路报警，4- 异常报警
            log.debug("2.门状态（或者梯控的楼层状态）：{}", netDvrAcsWorkStatusV50.byDoorStatus[0]); //门状态（或者梯控的楼层状态）：1- 休眠，2- 常开状态（对于梯控，表示自由状态），3- 常闭状态（对于梯控，表示禁用状态），4- 普通状态（对于梯控，表示受控状态）
            log.debug("3.门磁状态：{}", netDvrAcsWorkStatusV50.byMagneticStatus[0]); //门磁状态，0-正常关，1-正常开，2-短路报警，3-断路报警，4-异常报警
            log.debug("4.事件报警输入状态：{}", netDvrAcsWorkStatusV50.byCaseStatus[0]); //事件报警输入状态：0- 无输入，1- 有输入
        }
    }
    ...
    public static void remoteControlGate(int userID) {
        ...
        if (!b_gate) {
            log.debug("远程控门失败,err={}", AcsMain.hCNetSDK.NET_DVR_GetLastError());
            return;
        }
        log.debug("远程控门成功");
    }
